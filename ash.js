...
function sendMainMenu(chatId) {
    bot.sendMessage(chatId, '🎛️ <b>RADAR V12 — MENU PRINCIPAL</b>\n\nChoisissez une action :', {
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: [
                [
...
function sendHelpMessage() {
    var helpText = '📖 <b>COMMANDES DU RADAR V12</b>\n\n' +
        ...
        '/help → Cette aide';
    bot.sendMessage(TELEGRAM_CHAT_ID, helpText, { parse_mode: 'HTML' });
}
...
bot.on('callback_query', async function(query) {
    ...
    if (data === 'action_pause') {
        isPaused = true;
        bot.sendMessage(chatId, '⏸️ <b>Radar en pause.</b>\nUtilisez /menu pour reprendre.', { parse_mode: 'HTML' });
        return;
    }

    if (data === 'action_resume') {
        isPaused = false;
        bot.sendMessage(chatId, '▶️ <b>Radar repris !</b> Surveillance active.', { parse_mode: 'HTML' });
        return;
    }
    ...
    if (data === 'action_clear') {
        ...
        processedSignatures.clear();
        bot.sendMessage(chatId, '🧹 <b>Cache vidé :</b> ' + count + ' signatures supprimées.', { parse_mode: 'HTML' });
        return;
    }
    ...
    if (data === 'action_add') {
        ...
        bot.sendMessage(chatId, '➕ <b>AJOUTER UNE CIBLE</b>\n\nEnvoyez l\'adresse du wallet à surveiller :', {
            parse_mode: 'HTML',
    ...
    if (data === 'action_remove') {
        if (targetWallets.size === 0) {
            bot.sendMessage(chatId, '⚠️ Aucune cible à supprimer.');
            return;
        }
        ...
        bot.sendMessage(chatId, '🗑️ <b>SUPPRIMER UNE CIBLE</b>\n\nChoisissez le wallet à supprimer :', {
            parse_mode: 'HTML',
    ...
    if (data === 'action_rename') {
        if (targetWallets.size === 0) {
            bot.sendMessage(chatId, '⚠️ Aucune cible à renommer.');
            return;
        }
        var renameButtons = [];
        for (const [w, config] of targetWallets.entries()) {
        ...
        bot.sendMessage(chatId, '🏷️ <b>RENOMMER UNE CIBLE</b>\n\nChoisissez le wallet à renommer :', {
            parse_mode: 'HTML',
    ...
    if (data === 'action_blacklist') {
        ...
        bot.sendMessage(chatId, '🚫 <b>BLACKLISTER UN WALLET</b>\n\nEnvoyez l\'adresse du wallet à ignorer :', {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: '❌ Annuler', callback_data: 'action_cancel' }]] }
        });
        return;
    ...
    if (data === 'action_whitelist') {
        if (blacklist.size === 0) {
            bot.sendMessage(chatId, '⚠️ La blacklist est vide.');
            return;
        }
        var wlButtons = [];
        ...
        bot.sendMessage(chatId, '✅ <b>RETIRER DE LA BLACKLIST</b>\n\nChoisissez le wallet à retirer :', {
            parse_mode: 'HTML',
    ...
    if (data === 'action_cancel') {
        ...
        bot.sendMessage(chatId, '❌ Action annulée.');
        return;
    }
    ...
    if (data.startsWith('rm_')) {
        ...
        if (targetWallets.has(wallet)) {
            ...
            bot.sendMessage(chatId, '🗑️ <b>Cible supprimée :</b>\n<code>' + wallet + '</code>' + labelInfo, {
                parse_mode: 'HTML',
    ...
    if (data.startsWith('ren_')) {
        ...
        bot.sendMessage(chatId, '🏷️ <b>Renommer :</b> <code>' + shortAddr(walletToRename) + '</code>\nNom actuel : <b>' + currentName + '</b>\n\nEnvoyez le nouveau nom :', {
            parse_mode: 'HTML',
            reply_markup: { inline_keyboard: [[{ text: '❌ Annuler', callback_data: 'action_cancel' }]] }
        });
        return;
    ...
    if (data.startsWith('wl_')) {
        ...
        if (blacklist.has(walletToWl)) {
            ...
            saveBlacklist();
            bot.sendMessage(chatId, '✅ <b>Wallet retiré de la blacklist :</b>\n<code>' + walletToWl + '</code>', {
                parse_mode: 'HTML',
                reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
            });
        }
...
bot.on('message', function(msg) {
    ...
    if (pending.type === 'add') {
        if (pending.step === 'wallet') {
            try {
                ...
                bot.sendMessage(msg.chat.id, '✅ Wallet valide !\n\nEnvoyez le montant <b>minimum</b> (SOL) :', {
                    parse_mode: 'HTML',
                    reply_markup: { inline_keyboard: [[{ text: '❌ Annuler', callback_data: 'action_cancel' }]] }
                });
            } catch (e) {
                bot.sendMessage(msg.chat.id, '❌ Adresse invalide. Réessayez :');
            }
            return;
        }
        ...
        if (pending.step === 'min') {
            ...
            if (isNaN(min) || min < 0) {
                bot.sendMessage(msg.chat.id, '❌ Nombre invalide. Envoyez le minimum en SOL :');
                return;
            }
            pending.min = min;
            pending.step = 'max';
            pendingActions.set(chatId, pending);
            bot.sendMessage(msg.chat.id, 'Envoyez le montant <b>maximum</b> (SOL) :', {
                parse_mode: 'HTML',
                reply_markup: { inline_keyboard: [[{ text: '❌ Annuler', callback_data: 'action_cancel' }]] }
            });
            return;
        ...
        if (pending.step === 'max') {
            ...
            if (isNaN(max) || max <= pending.min) {
                bot.sendMessage(msg.chat.id, '❌ Le max doit être supérieur à ' + pending.min + '. Réessayez :');
                return;
            }
            pending.max = max;
            pending.step = 'label';
            pendingActions.set(chatId, pending);
            bot.sendMessage(msg.chat.id, 'Donnez un <b>nom</b> à cette cible (ou appuyez sur Passer) :', {
                parse_mode: 'HTML',
                reply_markup: {
                    inline_keyboard: [
    ...
    if (pending.type === 'blacklist') {
        try {
            ...
            if (blacklist.has(text)) {
                bot.sendMessage(msg.chat.id, '⚠️ Déjà dans la blacklist.');
            ...
            saveBlacklist();
            pendingActions.delete(chatId);
            bot.sendMessage(msg.chat.id, '🚫 <b>Wallet blacklisté :</b>\n<code>' + text + '</code>', {
                parse_mode: 'HTML',
                reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
            });
        } catch (e) {
    ...
    if (pending.type === 'rename') {
        ...
        if (targetWallets.has(walletRen)) {
            ...
            saveDatabase();
            pendingActions.delete(chatId);
            bot.sendMessage(msg.chat.id, '🏷️ <b>Wallet renommé :</b>\n<code>' + shortAddr(walletRen) + '</code> → <b>' + text + '</b>', {
                parse_mode: 'HTML',
                reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
            });
        }
...
bot.onText(/\/add (\S+) ([\d.]+) ([\d.]+)\s*(.*)/, function(msg, match) {
    ...
    if (min >= max) {
        bot.sendMessage(TELEGRAM_CHAT_ID, '❌ <b>Erreur :</b> Le min doit être inférieur au max.', { parse_mode: 'HTML' });
        return;
    }
    try {
        ...
        bot.sendMessage(TELEGRAM_CHAT_ID, '❌ <b>Erreur :</b> Adresse Solana invalide.', { parse_mode: 'HTML' });
    }
});
...
bot.onText(/\/rename (\S+) (.+)/, function(msg, match) {
    ...
    if (targetWallets.has(wallet)) {
        ...
        targetWallets.set(wallet, config);
        saveDatabase();
        bot.sendMessage(TELEGRAM_CHAT_ID, '🏷️ <b>Wallet renommé :</b>\n<code>' + wallet + '</code>\n→ <b>' + newLabel + '</b>', { parse_mode: 'HTML' });
    } else {
        bot.sendMessage(TELEGRAM_CHAT_ID, '⚠️ Ce wallet n\'est pas dans votre liste.');
    }
});
...
function sendStatusMessage() {
    if (targetWallets.size === 0) {
        bot.sendMessage(TELEGRAM_CHAT_ID, '📊 Aucun portefeuille surveillé.', {
            reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
        });
        return;
    }
...
function sendStatsMessage() {
    ...
    bot.sendMessage(TELEGRAM_CHAT_ID, stats, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
    });
}
...
bot.onText(/\/remove (\S+)/, function(msg, match) {
    ...
    if (targetWallets.has(wallet)) {
        ...
        bot.sendMessage(TELEGRAM_CHAT_ID, '🗑️ <b>Cible supprimée :</b>\n<code>' + wallet + '</code>' + labelInfo, { parse_mode: 'HTML' });
    } else {
        bot.sendMessage(TELEGRAM_CHAT_ID, '⚠️ Ce wallet n\'est pas dans votre liste.');
    }
});
...
bot.onText(/\/pause$/, function(msg) {
    if (!isAuthorized(msg)) return;
    isPaused = true;
    bot.sendMessage(TELEGRAM_CHAT_ID, '⏸️ <b>Radar en pause.</b>', { parse_mode: 'HTML' });
});

bot.onText(/\/resume$/, function(msg) {
    if (!isAuthorized(msg)) return;
    isPaused = false;
    bot.sendMessage(TELEGRAM_CHAT_ID, '▶️ <b>Radar repris !</b>', { parse_mode: 'HTML' });
});
...
bot.onText(/\/clear$/, function(msg) {
    ...
    var count = processedSignatures.size;
    processedSignatures.clear();
    bot.sendMessage(TELEGRAM_CHAT_ID, '🧹 <b>Cache vidé :</b> ' + count + ' signatures.', { parse_mode: 'HTML' });
});
...
bot.onText(/\/blacklist (\S+)/, function(msg, match) {
    ...
    try {
        ...
        if (blacklist.has(wallet)) {
            bot.sendMessage(TELEGRAM_CHAT_ID, '⚠️ Déjà dans la blacklist.');
            return;
        }
        blacklist.add(wallet);
        saveBlacklist();
        bot.sendMessage(TELEGRAM_CHAT_ID, '🚫 <b>Wallet blacklisté :</b>\n<code>' + wallet + '</code>', { parse_mode: 'HTML' });
    } catch (e) {
        bot.sendMessage(TELEGRAM_CHAT_ID, '❌ Adresse invalide.', { parse_mode: 'HTML' });
    }
});
...
bot.onText(/\/whitelist (\S+)/, function(msg, match) {
    ...
    if (blacklist.has(wallet)) {
        ...
        saveBlacklist();
        bot.sendMessage(TELEGRAM_CHAT_ID, '✅ <b>Retiré de la blacklist :</b>\n<code>' + wallet + '</code>', { parse_mode: 'HTML' });
    } else {
        bot.sendMessage(TELEGRAM_CHAT_ID, '⚠️ Pas dans la blacklist.');
    }
});
...
function sendBlacklistMessage() {
    if (blacklist.size === 0) {
        bot.sendMessage(TELEGRAM_CHAT_ID, '🚫 La blacklist est vide.', {
            reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
        });
        return;
    }
    var list = Array.from(blacklist).map(function(w) { return '• <code>' + w + '</code>'; }).join('\n');
    bot.sendMessage(TELEGRAM_CHAT_ID, '🚫 <b>BLACKLIST (' + blacklist.size + ')</b>\n\n' + list, {
        parse_mode: 'HTML',
        reply_markup: { inline_keyboard: [[{ text: '🔙 Menu', callback_data: 'action_menu' }]] }
    });
}
...
async function sendTG(message) {
    try {
        await bot.sendMessage(TELEGRAM_CHAT_ID, message, { parse_mode: 'HTML', disable_web_page_preview: true });
    } catch (e) {
...
async function analyzeHop(signature, sourceWallet, originalConfig) {
    ...
    try {
        ...
        if (!tx || !tx.transaction || !tx.transaction.message || !tx.transaction.message.instructions) {
            console.log('[🔍] Hop: tx null ou sans instructions pour ' + shortAddr(sourceWallet));
            return;
        }

        // Chercher dans les instructions principales ET les innerInstructions
        var allTransfers = [];
        ...
        for (var i = 0; i < instructions.length; i++) {
            ...
            if (ix.program === 'system' && ix.parsed && ix.parsed.type === 'transfer') {
                allTransfers.push(ix.parsed.info);
            }
        }

        // Inner instructions (transferts cachés dans des appels de programme)
        if (tx.meta && tx.meta.innerInstructions) {
            for (var j = 0; j < tx.meta.innerInstructions.length; j++) {
                ...
                if (inner.instructions) {
                    for (var k = 0; k < inner.instructions.length; k++) {
                        var innerIx = inner.instructions[k];
                        if (innerIx.program === 'system' && innerIx.parsed && innerIx.parsed.type === 'transfer') {
                            allTransfers.push(innerIx.parsed.info);
                        }
                    }
                }
            }
        ...
        console.log('[🔍] Hop: ' + allTransfers.length + ' transfert(s) trouvé(s) pour ' + shortAddr(sourceWallet));

        for (var t = 0; t < allTransfers.length; t++) {
            var info = allTransfers[t];
            if (info.source === sourceWallet) {
                ...
                // Pour les rebonds : on alerte sur tout transfert > 0.5 SOL
                // (pas besoin du filtre min/max strict, c'est pour détecter les rebonds)
                ...
                // Wallet neuf OU très faible balance (< 0.01 SOL)
                var isNewWallet = destIndex !== -1 && tx.meta &&
                    tx.meta.preBalances[destIndex] < 0.01 * solanaWeb3.LAMPORTS_PER_SOL;
                ...
                if (isNewWallet) {
                    ...
                    console.log('[🔍] Hop ignoré: ' + shortAddr(sourceWallet) + ' → ' + shortAddr(dest) + ' (' + amount.toFixed(4) + ' SOL, pas un nouveau wallet)');
                }
            }
        }
    } catch (e) {
...
function startTrackingNewWallet(wallet, originalConfig) {
    ...
    var subId = solanaConnection.onLogs(new solanaWeb3.PublicKey(wallet), async function(logs) {
        ...
        console.log('[📡] Activité détectée sur wallet dynamique : ' + shortAddr(wallet) + ' | sig: ' + logs.signature.slice(0, 12) + '...');
